...
#function to calculate angle between two points with respect to x-axis (horizontal axis)
def calcAngle(a, b):
    try:
        ax, ay = a
        bx, by = b
		...
		if (all_peaks[3][0][0:2]):
			try:
				if (all_peaks[4][0][0:2]):
					...
					if (distance < (armdist + 100) and distance > (armdist - 100) ): #this value 100 is arbitary.
...
def checkKneeling(all_peaks):
	...
	try:
		...
		if (leftdegrees > 60  and rightdegrees > 60): # 60 degrees is trail and error value here.
...
if __name__ == '__main__': #main function of the program
	...
	if(vi == False):
		...
		canvas, position= process('./input_images/Hand_not_fold_straight.jpeg', params, model_params)
		showimage(canvas)
		if (position == 1):
			print("Hunchback")
		elif (position == -1):
